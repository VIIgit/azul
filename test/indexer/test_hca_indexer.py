            ...
            for test_mode in ('0', '1'):
                for body in (testless_notification, test_name_in_notification):
                    with self.subTest(test_mode=test_mode, endpoint=endpoint):
                        with patch.dict(os.environ, TEST_MODE=test_mode):
                            ...
                            if 'test_name' not in body and test_mode == '1':
                                ...
                                self.assertEqual({
                                    ...
                                    "Message": "ChaliceViewError: Ignored notification {'match': {'bundle_uuid': "
                                               "'bb2365b9-5a5b-436f-92e3-4fc6d86a9efd', 'bundle_version': "
                                               "'2018-03-28T13:55:26.044Z'}}. This indexer is currently in TEST MODE."
                                }, response.json())
                            else:
                                self.assertEqual(202, response.status_code)
