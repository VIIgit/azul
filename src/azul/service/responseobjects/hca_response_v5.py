...
class ManifestResponse(AbstractResponse):
    ...
    def _construct_bdbag(self) -> str:
        ...
        """
        hits = list(self.es_search.scan())
        bundles = NestedOrderedDict()
        for hit_obj in hits:
            hit = hit_obj.to_dict()
            self._collect_bdbag_tsv_file_headers(hit, bundles)
    ...
    @staticmethod
    def _create_bdbag_tsv_file_header_row(data: NestedOrderedDict) -> list:
        row = []
    ...
    @staticmethod
    def _collect_bdbag_tsv_file_headers(hit: dict, data: NestedOrderedDict) -> NestedOrderedDict:
    ...
    def _put_bdbag_tsv_file_data_in_dict(self, hit: dict, data: NestedOrderedDict) -> NestedOrderedDict:
        for contents in hit.keys():
            # Bundle ID and version.
            if isinstance(hit[contents], list):
    ...
    @staticmethod
    def _construct_bdbag_tsv_file_data_row(data: NestedOrderedDict) -> list:
        row = []
